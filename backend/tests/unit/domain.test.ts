...
describe("Task 2 TypeScript domain contracts", () => {
  ...
  it("validates command envelopes and canonicalizes request hashes", () => {
    const first = parseCommand({
      ...
      payload: { name: "demo", constraints: {} },
    });
    const second = parseCommand({
      payload: { constraints: {}, name: "demo" },
    ...
    expect(() =>
      parseCommand({ ...first, payload: { commandId: "spoof" } }),
    ).toThrow(/reserved/);
  });
  ...
  it("requires attempt and redaction context for call/security events", () => {
    ...
    expect(
      parseEventDraft({
        ...
        redactionReason: "summary only",
      }).eventCategory,
    ).toBe("ordinary");
  });

  it("serializes stable domain errors without secrets", () => {
    const error = new InvalidArgumentError("apiKey=sk-test-secret", {
      data: { token: "secret-value" },
    });
    expect(error).toBeInstanceOf(DomainError);
    expect(error.message).not.toContain("sk-test-secret");
    expect(new VersionConflictError().toPayload().code).toBe(
      "VERSION_CONFLICT",
    );
  });
  ...
  it("allows dependency-free tasks and rejects coerced boolean fields", () => {
    const task = parseTask({
      id: "task_1",
      projectId: "project_1",
      title: "Task",
