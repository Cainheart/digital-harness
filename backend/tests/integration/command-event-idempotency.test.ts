...
describe("Task 2 events, outbox and command idempotency", () => {
  it("appends immutable events and an outbox message in one transaction", () => {
    ...
    const database = new Database(`${rootPath}/company.db`, {
      persistentRoot: rootPath,
    });
    database.initialize();
    ...
    const result = database.transaction((connection) =>
      events.append(connection, "project", project.id, 0, [draft(project.id)]),
    );
    ...
    expect(() =>
      database.connection
        .prepare("UPDATE domain_events SET result='changed' WHERE event_id=?")
        .run(result.events[0].eventId),
    ).toThrow();
    database.close();
  });
  ...
  it("returns the exact prior response for the same idempotency key", async () => {
    ...
    const project = makeProject();
    const repository = new ProjectTaskRepository();
    const service = new CommandService(app.runtime.database);
    ...
    const options = {
      ...
      stateWriter: (
        connection: Parameters<typeof repository.createProject>[0],
      ) => {
        repository.createProject(connection, project);
        return project;
      },
      eventDrafts: [draft(project.id, "ProjectStarted")],
    };
    const first = service.execute(command, options);
    const second = service.execute(command, options);
  ...
  it("persists call-event context fields instead of losing them on reload", () => {
    ...
    const event = {
      ...draft(project.id, "ModelCallRejected"),
      aggregateType: "project",
      attemptId: "attempt_1",
      rejectionReason: "policy denied",
      redactionReason: "secret removed",
      eventCategory: "security" as const,
    };
    const store = new SqliteEventStore();
    const appended = database.transaction((connection) =>
      store.append(connection, "project", project.id, 0, [event]),
    );
    const loaded = store.listForAggregate(
      database.connection,
      "project",
      project.id,
    )[0];
  ...
  it("rolls back state when event append fails and rejects stale versions", async () => {
    ...
    const repository = new ProjectTaskRepository();
    const service = new CommandService(app.runtime.database);
    const invalidCommand = parseCommand({
    ...
    expect(
      repository.findProject(app.runtime.database.connection, project.id),
    ).toBeNull();

    app.runtime.database.transaction((connection) =>
      repository.createProject(connection, project),
    );
    const first = app.runtime.database.transaction((connection) =>
      new SqliteEventStore().append(connection, "project", project.id, 0, [
        draft(project.id),
      ]),
    );
    const staleCommand = parseCommand({
      commandId: "cmd_stale",
    ...
    expect(() =>
      service.execute(staleCommand, {
        aggregateType: "project",
        stateWriter: () => project,
        eventDrafts: [draft(project.id, "ProjectUpdated")],
      }),
    ).toThrow(/VERSION_CONFLICT|版本冲突/);
    await app.close();
  });
});
