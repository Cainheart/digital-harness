...
describe("Task 2 evidence and trace repositories", () => {
  it("stores content-addressed artifacts and detects tampering", async () => {
    ...
    const reference = await app.runtime.artifactStore.put(
      Buffer.from("hello"),
      "text/plain",
      { projectId: "project_1", artifactId: "artifact_1" },
    );
    expect((await app.runtime.artifactStore.verify(reference)).valid).toBe(
      true,
    );
  ...
  it("persists v1/v2 ArtifactVersion history, latest Artifact projection and integrity status", async () => {
    ...
    const ref1 = await app.runtime.artifactStore.put(
      Buffer.from("report v1"),
      "text/plain",
    ...
    const version1: ArtifactVersion = {
      ...
      contentRef: {
        artifactId: artifact.id,
        sha256: ref1.sha256,
        mediaType: ref1.mediaType,
        size: ref1.sizeBytes,
        createdAt: ref1.createdAt,
        relativePath: ref1.relativePath,
        storeRef: ref1.storeRef,
      },
      storeRef: ref1.storeRef,
      createdAt: utcNow(),
      createdBy: "developer",
    ...
    const ref2 = await app.runtime.artifactStore.put(
      Buffer.from("report v2"),
      "text/plain",
      { projectId: project.id, artifactId: artifact.id },
    );
    const version2: ArtifactVersion = {
      ...
      contentRef: {
        ...
        relativePath: ref2.relativePath,
        storeRef: ref2.storeRef,
      },
      storeRef: ref2.storeRef,
      createdAt: utcNow(),
      createdBy: "developer",
    ...
    app.runtime.database.transaction((connection) => {
      evidence.createArtifactVersion(connection, version1);
      evidence.createArtifactVersion(connection, version2);
    });

    const versions = evidence.listArtifactVersions(
    ...
    expect(
      evidence.getArtifact(app.runtime.database.connection, artifact.id)
        .contentRef?.sha256,
    ).toBe(ref2.sha256);
    expect(
      evidence.getArtifact(app.runtime.database.connection, artifact.id)
    ...
    const verified = await evidence.verifyArtifactVersion(
      app.runtime.database.connection,
    ...
    expect(
      evidence.getArtifactVersion(app.runtime.database.connection, version2.id)
        .integrityStatus,
    ).toBe("verified");
    const tamperedPath = `${app.runtime.artifactStore.root}/${ref2.relativePath}`;
    const fs = await import("node:fs");
    fs.writeFileSync(tamperedPath, "tampered v2");
    const invalid = await evidence.verifyArtifactVersion(
      app.runtime.database.connection,
      app.runtime.artifactStore,
      version2.id,
    );
  ...
  it("supports valid Artifact TraceLinks in both directions and rejects cross-project links", async () => {
    ...
    expect(
      trace.coverage(app.runtime.database.connection, project.id, "artifact", [
        artifact.id,
        "artifact_missing",
      ]),
    ).toEqual({
