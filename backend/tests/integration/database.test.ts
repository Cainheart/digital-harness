...
describe("Task 1 runtime and SQLite schema", () => {
  it("initializes persistent layout, Task 1/2 tables and WAL", () => {
    ...
    const root = new PersistenceRoot(rootPath, { appVersion: "0.1.0", schemaRevision: SUPPORTED_SCHEMA_REVISION });
    const database = new Database(root.databasePath, { persistentRoot: root.root });
  ...
  it("keeps committed project/task data after reopening", () => {
    const rootPath = useTestRoot();
    const root = new PersistenceRoot(rootPath, { appVersion: "0.1.0", schemaRevision: SUPPORTED_SCHEMA_REVISION });
    const first = new Database(root.databasePath, { persistentRoot: root.root });
    root.initializeDatabase(first);
    const repository = new ProjectTaskRepository();
    const project = makeProject();
    const task = makeTask(project.id);
    first.transaction((connection) => { repository.createProject(connection, project); repository.createTask(connection, task); });
    first.close();
    const second = new Database(root.databasePath, { persistentRoot: root.root });
    expect(repository.getProject(second.connection, project.id).name).toBe(project.name);
    expect(repository.getTask(second.connection, task.id).status).toBe("待处理");
    second.close();
  });
  ...
  it("exposes all five readiness checks and blocks real execution without side effects", async () => {
    ...
    const readiness = await app.inject({ method: "GET", url: "/api/v1/readiness" });
    const view = readiness.json() as { status: string; allowedActions: string[]; checks: Record<string, { status: string; impact?: string; nextAction?: string }> };
    expect(readiness.statusCode).toBe(200);
    expect(Object.keys(view.checks).sort()).toEqual(["docker", "model", "persistence", "research", "workspace"]);
    ...
    for (const name of ["model", "docker", "research"]) expect(view.checks[name]?.impact && view.checks[name]?.nextAction).toBeTruthy();
    const executionEventsBefore = app.runtime.database.executionEventCount();
    const error = await app.runtime.startupGate.tryAssertReady("tr_startup_guard");
    expect(error?.code).toBe("WORKFLOW_GUARD_BLOCKED");
    expect(app.runtime.database.executionEventCount()).toBe(executionEventsBefore);
    await app.close();
  });
  ...
  it("returns blocked readiness, refuses non-loopback requests and audits the denial", async () => {
    const rootPath = useTestRoot();
    const app = await createTestApp(rootPath);
    const denied = await app.inject({ method: "GET", url: "/api/v1/readiness", headers: { "x-test-remote-address": "192.0.2.10" } });
    expect(denied.statusCode).toBe(403);
    expect(denied.json().code).toBe("POLICY_DENIED");
    expect((app.runtime.database.connection.prepare("SELECT COUNT(*) AS count FROM runtime_events WHERE event_type='SecurityAccessDenied'").get() as { count: number }).count).toBe(1);
    await app.close();
  });
});
