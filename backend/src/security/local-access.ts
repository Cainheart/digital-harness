...
export function trustedClientHost(
  request: {
    ip: string;
  ...
  if (testMode) {
    const injected = request.headers["x-test-remote-address"];
    return typeof injected === "string" ? injected : "127.0.0.1";
  }
  return request.ip;
}
...
export function assertLocalRequest(
  ...
  const host = trustedClientHost(request, testMode);
  ...
  if (!loopback) {
    throw new RuntimeBoundaryError({
      ...
      dataPreserved: true,
      nextAction: "从运行产品的本机访问，或检查监听地址配置",
      traceId,
      statusCode: 403,
    });
  }
...
/** 识别 IPv4、IPv6 和 IPv4-mapped IPv6 的本机回环地址。 */
