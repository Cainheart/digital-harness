...
export class MemoryCredentialAdapter implements CredentialAdapter {
  ...
  async save(provider: string, secret: string): Promise<string> {
    const ref = `memory://${provider}/${randomUUID()}`;
    this.values.set(ref, { provider, secret });
    return ref;
  }
  /** 在实际调用边界读取短时租约。 */
  async read(secretRef: string): Promise<SecretLease> {
    const item = this.values.get(secretRef);
    if (!item) throw new Error("credential unavailable");
    return new SecretLease(item.secret);
  }
  /** 删除进程内凭据。 */
  ...
  /** 只返回是否存在且非空。 */
  async check(secretRef: string): Promise<CredentialCheckResult> {
    const item = this.values.get(secretRef);
    return item?.secret
      ? { available: true, message: "credential available" }
      : { available: false, message: "credential unavailable" };
  }
  /** 清空测试替身中的全部明文。 */
  clear(): void {
    this.values.clear();
...
export class SystemCredentialAdapter implements CredentialAdapter {
  ...
  constructor(service = "digital-harness") {
  ...
  async save(provider: string, secret: string): Promise<string> {
    ...
    if (process.platform !== "darwin")
      throw new Error("OS Keychain adapter requires a platform implementation");
    ...
    await new Promise<void>((resolve, reject) =>
      execFile(
        ...
        (error) =>
          error ? reject(new Error("OS Keychain save failed")) : resolve(),
      ),
    );
    return ref;
  }
  ...
  async read(secretRef: string): Promise<SecretLease> {
    ...
    return new SecretLease(value.trim());
  }
  ...
  async check(secretRef: string): Promise<CredentialCheckResult> {
    try {
      ...
      return { available: true, message: "credential available" };
    } catch (_error) {
      return { available: false, message: "credential unavailable" };
    }
  }
  private async runSecurity(args: string[]): Promise<string> {
    ...
    return new Promise((resolve, reject) =>
      execFile("security", args, { encoding: "utf8" }, (error, stdout) =>
        error
          ? reject(new Error("OS Keychain operation failed"))
          : resolve(stdout),
      ),
    );
  }
