...
/** V1 只有本机 Boss 身份可以作为 Boss 端点，员工消息不能伪造其他 Boss 实例。 */
const LOCAL_BOSS_INSTANCE_ID = "boss-local";
/** 结构化消息允许引用的业务对象；未知对象类型默认拒绝，避免伪造 TraceLink。 */
const TRACEABLE_OBJECTS: Record<string, { table: string; idColumn: string }> = { project: { table: "projects", idColumn: "id" }, task: { table: "tasks", idColumn: "id" }, artifact: { table: "artifacts", idColumn: "id" }, artifact_version: { table: "artifact_versions", idColumn: "id" }, approval: { table: "approvals", idColumn: "id" }, review: { table: "reviews", idColumn: "id" }, test_case: { table: "test_cases", idColumn: "id" }, test_run: { table: "test_runs", idColumn: "id" }, defect: { table: "defects", idColumn: "id" }, execution_attempt: { table: "execution_attempts", idColumn: "id" }, message: { table: "structured_messages", idColumn: "message_id" } };
...
export class StructuredMessageRepository {
  ...
  assertEndpoint(connection: BetterSqlite3.Database, endpoint: { roleId: string; instanceId: string }, name: string): OrganizationMember {
    if (canonicalRoleId(endpoint.roleId) === "boss" && endpoint.instanceId === LOCAL_BOSS_INSTANCE_ID) return { instanceId: endpoint.instanceId, roleId: "boss", displayName: "Boss", specialistTag: "boss", officeZone: "boss", deskGroup: "boss", status: "available", roleVersion: 1 };
    const row = connection.prepare("SELECT organization_members.*, role_definitions.enabled AS role_enabled, role_definitions.role_version AS current_role_version FROM organization_members JOIN role_definitions ON role_definitions.role_id=organization_members.role_id WHERE organization_members.instance_id=?").get(endpoint.instanceId) as { instance_id: string; role_id: string; display_name: string; specialist_tag: string; office_zone: string; desk_group: string; status: "available" | "busy" | "blocked"; role_version: number; role_enabled: number; current_role_version: number } | undefined;
    if (!row || row.role_enabled !== 1 || row.current_role_version !== row.role_version || canonicalRoleId(row.role_id) !== canonicalRoleId(endpoint.roleId)) throw new InvalidMessageError(`${name} 岗位或员工实例无效`, { data: { instanceId: endpoint.instanceId, roleId: endpoint.roleId } });
  ...
  create(connection: BetterSqlite3.Database, input: CreateMessageInput): { message: StructuredMessage; created: boolean } {
    ensureProjectWritable(connection, input.projectId); ensureProjectChild(connection, "tasks", input.projectId, input.taskId); this.assertEndpoint(connection, input.sender, "sender"); this.assertEndpoint(connection, input.receiver, "receiver"); if (input.sourceObjectType && input.sourceObjectId) this.assertObjectReference(connection, input.projectId, input.sourceObjectType, input.sourceObjectId, "source"); if (input.responseObjectType && input.responseObjectId) this.assertObjectReference(connection, input.projectId, input.responseObjectType, input.responseObjectId, "response");
  ...
  // 修改日期：2026-08-16
  // 修改原因：重复确认不能重复推进消息版本或追加领域事件；同时以 UPDATE changes 作为并发竞争下的最终判定。
  acknowledge(connection: BetterSqlite3.Database, messageId: string, handledBy: string): { message: StructuredMessage; changed: boolean } { const row = connection.prepare("SELECT * FROM structured_messages WHERE message_id=?").get(messageId) as MessageRow | undefined; if (!row) throw new NotFoundError("结构化消息不存在"); if (row.status !== "pending") return { message: messageFromRow(row), changed: false }; const update = connection.prepare("UPDATE structured_messages SET status='acknowledged',handled_at=?,handled_by=?,version=version+1 WHERE message_id=? AND status='pending'").run(new Date().toISOString(), handledBy, messageId); const updated = connection.prepare("SELECT * FROM structured_messages WHERE message_id=?").get(messageId) as MessageRow; return { message: messageFromRow(updated), changed: update.changes === 1 }; }
  /** 校验消息原始对象和响应对象存在且属于同一项目。 */
  // 修改日期：2026-08-16
  // 修改原因：只保存 objectType/objectId 不能证明交接真实存在；跨项目或不存在的引用会破坏需求、任务和审批的可追踪链。
  private assertObjectReference(connection: BetterSqlite3.Database, projectId: string, objectType: string, objectId: string, name: string): void { const descriptor = TRACEABLE_OBJECTS[objectType]; if (!descriptor) throw new InvalidMessageError(`${name} 对象类型不支持`, { data: { objectType } }); const row = descriptor.table === "projects" ? connection.prepare("SELECT id AS project_id FROM projects WHERE id=?").get(objectId) as { project_id: string } | undefined : connection.prepare(`SELECT project_id FROM ${descriptor.table} WHERE ${descriptor.idColumn}=?`).get(objectId) as { project_id: string } | undefined; if (!row || row.project_id !== projectId) throw new InvalidMessageError(`${name} 对象不存在或不属于当前项目`, { data: { objectType, objectId, projectId } }); }
}
...
// 修改日期：2026-08-16
// 修改原因：客户端对象键顺序不属于业务语义；幂等重试必须只比较规范化后的请求内容，避免合法重试被误报为冲突。
function messageHash(input: CreateMessageInput): string { return createHash("sha256").update(JSON.stringify(sortKeys({ ...input, sender: { ...input.sender, roleId: canonicalRoleId(input.sender.roleId) }, receiver: { ...input.receiver, roleId: canonicalRoleId(input.receiver.roleId) } }))).digest("hex"); }
/** 递归排序消息 JSON 键，避免同一幂等请求因客户端字段顺序不同而被误判为冲突。 */
function sortKeys(value: unknown): unknown { if (Array.isArray(value)) return value.map(sortKeys); if (value && typeof value === "object") return Object.fromEntries(Object.entries(value).sort(([left], [right]) => left.localeCompare(right)).map(([key, item]) => [key, sortKeys(item)])); return value; }
