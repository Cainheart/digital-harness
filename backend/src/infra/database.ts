...
import { redact, redactJson } from "../security/redaction.js";
import {
  migrateDomainSchema,
...
export type MigrationBackupContext = {
...
export type BackupReceipt = {
  ...
  sourceSchemaRevision: string;
  targetSchemaRevision: string;
  fileManifest: Record<string, { sha256: string; size: number }>;
  safetyStatus: Record<string, string>;
  verified: boolean;
  persistentRoot?: string;
  databasePath?: string;
};
...
export function validateNoFollowPath(input: string): string {
  ...
  for (const part of parts) {
    ...
    if (
      stat.isSymbolicLink() ||
      (!stat.isDirectory() && current !== absolute) ||
      stat.isFIFO() ||
      stat.isSocket() ||
...
export class Database {
  ...
  constructor(
    ...
    try {
      validateSchemaRevision(
        options.schemaRevision ?? SUPPORTED_SCHEMA_REVISION,
      );
    } catch (_error) {
      throw runtimeError(
        "SCHEMA_CONFIGURATION_INVALID",
        `应用只支持 ${SUPPORTED_SCHEMA_REVISION} Schema 基线`,
        "修正 currentSchemaRevision 配置后重试",
        SUPPORTED_SCHEMA_REVISION,
      );
    }
    this.path = canonicalPath(path);
    this.persistentRoot = canonicalPath(
      options.persistentRoot ?? dirname(this.path),
    );
    this.appVersion = options.appVersion ?? "0.1.0";
    try {
      if (this.path !== join(this.persistentRoot, "company.db"))
        throw new Error(
          "database path must be company.db under persistent root",
        );
      ...
      this.connection.function(
        ...
        (projectId: string | null) =>
          projectId !== null && projectId === this.purgeProjectId ? 1 : 0,
      );
      this.orm = drizzle(this.connection);
    } catch (_error) {
      throw runtimeError(
        "SCHEMA_CONFIGURATION_INVALID",
        "数据库路径必须是持久化根目录内的 company.db，且边界路径不得为符号链接",
        "修正 persistentRoot/company.db 路径和持久化目录后重试",
        SUPPORTED_SCHEMA_REVISION,
      );
    }
  }
  ...
  initialize(backupCallback?: BackupCallback): void {
    try {
      ...
      if (current === null) {
        ...
        if (tables.size > 0 && !tables.has("drizzle_migrations"))
          throw this.schemaConflict(
            null,
            "备份持久化根目录并沿批准路径恢复或执行 Schema migration",
          );
        migrateRuntimeSchema(this.connection);
        ...
        if (
          ...
          throw this.persistenceError(
            ...
            "修复持久化根目录备份并重新执行批准的 Schema migration",
          );
        migrateDomainSchema(this.connection);
        migrateIntegritySchema(this.connection);
        migrateOrganizationSchema(this.connection);
      ...
      throw this.persistenceError(
        ...
        "检查 SQLite 文件、锁状态和批准 migration 日志后重试",
      );
    }
  }
  ...
  currentRevision(): string | null {
    ...
    try {
      ...
      return row?.version_num ?? null;
    } catch (_error) {
      throw this.persistenceError(
        "SCHEMA_READ_FAILED",
        "检查 SQLite 文件、锁状态和 migration 日志后重试",
      );
    }
  }
  ...
  checkSchema(): SchemaCheckResult {
    ...
    if (revision !== SUPPORTED_SCHEMA_REVISION)
      return {
        ...
        message:
          "Database schema revision is incompatible with this application",
        dataPreserved: true,
        nextAction:
          "Back up the persistent root and apply the approved migration path",
      };
    try {
      this.ensureSchemaContract();
      ...
      if (journalMode.toLowerCase() !== "wal")
        return {
          ...
          message: "SQLite WAL 未启用",
          dataPreserved: true,
          nextAction: "检查 SQLite 数据库和持久化根目录",
        };
      return {
        writable: true,
        revision,
      ...
      if (error instanceof RuntimeBoundaryError)
        return {
          ...
          dataPreserved: error.dataPreserved,
      ...
      return {
        ...
        message: "Schema integrity contract is incompatible",
        dataPreserved: true,
        nextAction: "恢复完整 Schema 和关键 trigger 后重试",
      };
    }
  }
  ...
  journalMode(): string {
    return String(this.connection.pragma("journal_mode", { simple: true }));
  }
  /** 以事务方式执行一组状态、事件和 outbox 写入。 */
  transaction<T>(callback: (connection: BetterSqlite3.Database) => T): T {
    return this.connection.transaction(callback)(this.connection);
  }
  /** 以项目删除授权状态执行受控 purge。 */
  controlledProjectPurge<T>(
    projectId: string,
    callback: (connection: BetterSqlite3.Database) => T,
  ): T {
    this.purgeProjectId = projectId;
    try {
      return this.transaction(callback);
    } finally {
      this.purgeProjectId = null;
    }
  }
  ...
  saveCredentialConfig(
    ...
    this.transaction((db) =>
      db
        .prepare(
          "INSERT INTO credential_configs (provider,model,secret_ref,config_version,connection_status,created_at,updated_at) VALUES (?,?,?,?,?,?,?)",
        )
        .run(
          provider,
          model,
  ...
  /** 追加一条带 trace 的运行事实事件。 */
  appendEvent(eventType: string, traceId: string, payload: string): void {
    ...
    this.transaction((db) =>
      db
        ...
        .run(eventType, traceId, safePayload, new Date().toISOString()),
    );
  }
  /** 读取运行事件文本供脱敏扫描和安全测试使用。 */
  readEventText(): string {
    return (
      this.connection
        .prepare("SELECT payload FROM runtime_events ORDER BY id")
  ...
  /** 原子替换当前运行状态，供重启恢复读取。 */
  writeRuntimeState(status: string, reason: string): void {
    const safeReason = redact(reason);
    this.transaction((db) => {
      db.prepare("DELETE FROM runtime_state").run();
      db.prepare(
        "INSERT INTO runtime_state (status,reason,updated_at) VALUES (?,?,?)",
      ).run(status, safeReason, new Date().toISOString());
    });
  }
  /** 读取最近一次运行状态。 */
  readRuntimeState(): {
    ...
    return (
      (this.connection
        .prepare(
          "SELECT status,reason,updated_at FROM runtime_state ORDER BY id DESC LIMIT 1",
        )
        .get() as
  ...
  executionEventCount(): number {
    return (
      this.connection
        .prepare(
          "SELECT COUNT(*) AS count FROM runtime_events WHERE event_type LIKE '%Execution%'",
        )
        .get() as { count: number }
    ).count;
  }
  /** 保存 Worker 最新租约。 */
  saveWorkerLease(workerId: string, heartbeatAt: string, status: string): void {
    this.transaction((db) => {
      db.prepare("DELETE FROM worker_leases WHERE worker_id=?").run(workerId);
      db.prepare(
        "INSERT INTO worker_leases (worker_id,heartbeat_at,status) VALUES (?,?,?)",
      ).run(workerId, heartbeatAt, status);
    });
  }
  /** 读取全部 Worker 租约。 */
  readWorkerLeases(): {
    ...
    return this.connection
      .prepare(
        "SELECT worker_id,heartbeat_at,status FROM worker_leases ORDER BY worker_id",
      )
      .all() as { worker_id: string; heartbeat_at: string; status: string }[];
  }
  ...
  deleteHistoricalProject(projectId: string, actorId: string): void {
    ...
    this.controlledProjectPurge(projectId, (db) => {
      ...
      db.prepare(
        "INSERT INTO project_deletion_audits (project_id,deleted_at,actor_id) VALUES (?,?,?)",
      ).run(projectId, now, actorId);
    });
  }
  /** 判断项目是否存在。 */
  ...
  private validateStorageDirectories(): void {
    for (const name of ["artifacts", "traces", "workspaces", "backups"]) {
      const path = join(this.persistentRoot, name);
      validateNoFollowPath(path);
      if (existsSync(path) && !lstatSync(path).isDirectory())
        throw new Error(`unsafe persistent directory: ${name}`);
    }
    for (const path of [
      this.path,
      `${this.path}-wal`,
      `${this.path}-shm`,
      join(this.persistentRoot, "manifest.json"),
    ]) {
      validateNoFollowPath(path);
  ...
  private ensureSchemaContract(): void {
    ...
    if (![...required].every((name) => tables.has(name)))
      throw this.schemaConflict(
        SUPPORTED_SCHEMA_REVISION,
        "恢复完整 Schema 或沿批准 migration 修复后重试",
    ...
    const counts = this.connection
      .prepare(
        "SELECT (SELECT COUNT(*) FROM organization_domains) AS domains, (SELECT COUNT(*) FROM role_definitions) AS roles, (SELECT COUNT(*) FROM organization_members) AS members",
      )
      .get() as { domains: number; roles: number; members: number };
    if (counts.domains < 5 || counts.roles < 19 || counts.members < 19)
      throw this.schemaConflict(
        SUPPORTED_SCHEMA_REVISION,
    ...
    const artifactVersionColumns = this.connection
      ...
      .all() as { name: string }[];
    if (
      !artifactVersionColumns.some(
        (column) => column.name === "integrity_status",
      )
    )
      throw this.schemaConflict(
        SUPPORTED_SCHEMA_REVISION,
        "恢复 artifact_versions.integrity_status 后重试",
        "SCHEMA_INTEGRITY_CONFLICT",
      );
    ...
    for (const name of [
      ...
      if (!names.has(name))
        throw this.schemaConflict(
          SUPPORTED_SCHEMA_REVISION,
          "恢复关键 immutable/TraceLink trigger 后重试",
          "SCHEMA_INTEGRITY_CONFLICT",
        );
    for (const [table, index] of Object.entries(PROJECT_ID_INDEX_NAMES)) {
    ...
    for (const [table, index, columns] of ARCHIVE_INDEX_DEFINITIONS) {
      ...
      if (!found || actualColumns !== columns)
        throw this.schemaConflict(
          SUPPORTED_SCHEMA_REVISION,
          `恢复 ${table} 的 Task 9 查询索引 ${index} 后重试`,
          "SCHEMA_INTEGRITY_CONFLICT",
        );
    }
  }
  ...
  private ensureColumns(table: string, columns: string[]): void {
    const actual = new Set(
      (
        this.connection.prepare(`PRAGMA table_info(${table})`).all() as {
          name: string;
        }[]
      ).map((column) => column.name),
    );
    ...
    if (missing.length > 0)
      throw this.schemaConflict(
        SUPPORTED_SCHEMA_REVISION,
        `恢复 ${table}.${missing.join(",")} 后重试`,
        "SCHEMA_INTEGRITY_CONFLICT",
      );
  }
  private schemaConflict(
    revision: string | null,
    nextAction: string,
    code = "VERSION_CONFLICT",
    ...
    return runtimeError(
      code,
      code === "VERSION_CONFLICT"
        ? "VERSION_CONFLICT: 当前数据库 Schema revision 不兼容"
  ...
  private persistenceError(
    ...
    return runtimeError(
      ...
      code === "MIGRATION_BACKUP_FAILED"
        ? "迁移前一致性备份未通过验证"
        : "持久化数据库当前不可用或 Schema migration 未完成",
      nextAction,
    );
  }
  /**
  ...
  private createPreMigrationBackup(
    ...
    const backupId = `migration-${randomUUID()}`;
    const root = join(context.persistentRoot, "backups", backupId);
    ...
    const copy = (source: string, destination: string): void => {
      ...
      if (data.length > BACKUP_MAX_FILE_SIZE)
        throw new Error("backup file too large");
      mkdirSync(dirname(destination), { recursive: true });
      writeFileSync(destination, data, { mode: 0o600 });
      manifest[destination.slice(root.length + 1)] = {
        sha256: createHash("sha256").update(data).digest("hex"),
        size: data.length,
      };
    };
    copy(context.databasePath, join(root, "database", "company.db"));
    for (const dir of BACKUP_DATA_DIRECTORIES) {
      if (!existsSync(join(context.persistentRoot, dir))) continue;
      for (const file of walkFiles(join(context.persistentRoot, dir)))
        copy(file, join(root, file.slice(context.persistentRoot.length + 1)));
    }
    writeFileSync(
    ...
    return {
      backupId,
      root,
      sourceSchemaRevision: context.sourceSchemaRevision,
      targetSchemaRevision: context.targetSchemaRevision,
      fileManifest: manifest,
      safetyStatus: {
        symlinkScan: "passed",
        specialFileScan: "passed",
        sensitiveContentScan: "secretRef-only database contract",
        credentialRedaction: "not applied to binary SQLite",
      },
      verified: true,
      persistentRoot: context.persistentRoot,
...
function runtimeError(
  ...
  return new RuntimeBoundaryError({
    ...
    dataPreserved: true,
    nextAction,
    traceId: `tr_${randomUUID().replaceAll("-", "").slice(0, 16)}`,
    schemaRevision,
  });
}
/** 递归列出常规文件，不跟随符号链接。 */
function walkFiles(root: string): string[] {
  ...
  for (const entry of readdirSync(root)) {
    ...
    if (stat.isSymbolicLink())
      throw new Error("backup tree contains a symlink");
    if (stat.isDirectory()) result.push(...walkFiles(path));
    else if (stat.isFile()) result.push(path);
    else throw new Error("backup tree contains a special file");
  }
  return result;
}
...
/** 只允许数据库保存凭据引用，不允许把明文密钥伪装成 secretRef。 */
function validateSecretReference(secretRef: string): void {
  if (
    !secretRef.trim() ||
    /[\u0000-\u001f\u007f\s]/.test(secretRef) ||
    /(?:sk-[A-Za-z0-9]|Bearer\s+)/i.test(secretRef)
  ) {
    throw new Error("secretRef must be a non-sensitive reference");
  }
}

/** 对 JSON 运行事件按字段名和常见凭据模式做脱敏，保留事件可查询性。 */
function sanitizeEventPayload(payload: string): string {
  try {
    return redactJson(JSON.parse(payload));
  } catch (_error) {
