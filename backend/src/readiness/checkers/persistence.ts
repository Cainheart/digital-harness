...
export class PersistenceReadinessChecker {
  ...
  /** 绑定数据库只读检查器，确保 readiness 不会创建业务数据。 */
  constructor(private readonly database: Database) {}
  /** 执行数据库版本与 journal mode 检查，不修改业务数据。 */
  async check(): Promise<CheckView> {
    try {
      const schema = this.database.checkSchema();
      const details = {
        journalMode: schema.writable
          ? this.database.journalMode()
          : "blocked-read-only",
      ...
      return schema.writable
        ? {
            ...
            schemaRevision: schema.revision ?? undefined,
            details,
          }
        : {
            status: "blocked",
            message: schema.message,
            code: schema.code ?? undefined,
            impact: "业务数据无法安全持久化",
            dataPreserved: schema.dataPreserved,
            schemaRevision: schema.revision ?? undefined,
            nextAction: schema.nextAction ?? "检查 SQLite 数据库和持久化根目录",
            details,
          };
    } catch (error) {
      const message =
