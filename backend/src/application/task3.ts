...
export class Task3OrganizationService {
  ...
  replaceRole(role: Parameters<OrganizationRepository["replaceRole"]>[1]): ReturnType<OrganizationRepository["replaceRole"]> { return this.database.transaction((connection) => { const before = this.organizationRepository.getRole(connection, role.roleId); const result = this.organizationRepository.replaceRole(connection, role); connection.prepare("INSERT INTO runtime_events (event_type,trace_id,payload,occurred_at) VALUES (?,?,?,?)").run("RolePolicyVersionChanged", `tr_role_policy_${role.roleId}_${role.roleVersion}`, JSON.stringify({ roleId: role.roleId, beforeRoleVersion: before.roleVersion, afterRoleVersion: role.roleVersion }), utcNow()); return result; }); }
  /** 启用岗位前执行完整性检查；任一职责字段为空都会返回 INVALID_ROLE_DEFINITION。 */
  enableRole(roleId: string): ReturnType<OrganizationRepository["enableRole"]> { return this.database.transaction((connection) => { const result = this.organizationRepository.enableRole(connection, roleId); connection.prepare("INSERT INTO runtime_events (event_type,trace_id,payload,occurred_at) VALUES (?,?,?,?)").run("RoleEnabled", `tr_role_enabled_${result.roleId}`, JSON.stringify({ roleId: result.roleId, roleVersion: result.roleVersion }), utcNow()); return result; }); }
  /** 禁用岗位并留下安全审计；已存在的历史 Attempt 不被修改。 */
  disableRole(roleId: string): void { this.database.transaction((connection) => { this.organizationRepository.disableRole(connection, roleId); connection.prepare("INSERT INTO runtime_events (event_type,trace_id,payload,occurred_at) VALUES (?,?,?,?)").run("RoleDisabled", `tr_role_disabled_${canonicalRoleId(roleId)}`, JSON.stringify({ roleId: canonicalRoleId(roleId) }), utcNow()); }); }
  ...
  acknowledgeMessage(messageId: string, handledBy: string): StructuredMessage { return this.database.transaction((connection) => { const result = this.messageRepository.acknowledge(connection, messageId, handledBy); if (result.changed) { const expectedVersion = this.eventStore.countForAggregate(connection, "structured_message", result.message.messageId); this.eventStore.append(connection, "structured_message", result.message.messageId, expectedVersion, [this.messageEvent(result.message, "StructuredMessageAcknowledged")]); } return result.message; }); }
...
const DIRECTION_LEAD_ROLES = new Set(["product_solution_pm", "developer_representative", "npi_lead", "test_lead", "process_supervisor", "quality_risk_supervisor"]);
...
export class BossDirectionService {
  ...
  convert(input: { approvalId: string; directionOpinion: string; assignedLead: { roleId: string; instanceId: string }; responseArtifactRequired?: boolean }): { approvalId: string; directionOpinion: string; assignedLead: { roleId: string; instanceId: string }; responseTaskId: string; responseArtifactRequired: boolean; messageId: string } {
    if (!input.directionOpinion.trim()) throw new InvalidArgumentError("directionOpinion 不能为空");
    return this.database.transaction((connection) => {
      const approval = this.evidence.getApproval(connection, input.approvalId);
      if (approval.responseTaskId) return this.readExistingConversion(connection, approval, input.directionOpinion);
      const lead = this.organization.getMember(connection, input.assignedLead.instanceId);
      this.assertLead(lead.roleId, input.assignedLead.roleId);
      const responseArtifactRequired = input.responseArtifactRequired !== false;
      const task: Task = { id: newObjectId("task_direction"), projectId: approval.projectId, title: `落实 Boss 方向：${input.directionOpinion.slice(0, 80)}`, ownerRole: canonicalRoleId(lead.roleId), specialistTag: lead.specialistTag, assignmentReason: `approval:${approval.id}`, priority: "P1", dependencies: approval.taskId ? [approval.taskId] : [], expectedDeliverables: responseArtifactRequired ? ["direction-response", "response-artifact"] : ["direction-response"], status: TaskStatus.PENDING, createdAt: utcNow(), startedAt: null, endedAt: null, version: 1 };
      this.projects.createTask(connection, task);
      const now = utcNow();
      connection.prepare("UPDATE approvals SET direction=?,response_task_id=?,status=?,decided_at=?,version=version+1 WHERE id=? AND response_task_id IS NULL").run(input.directionOpinion, task.id, "direction_assigned", now, approval.id);
      const messageInput: CreateMessageInput = { sender: { roleId: "boss", instanceId: approval.bossId }, receiver: { roleId: lead.roleId, instanceId: lead.instanceId }, projectId: approval.projectId, taskId: task.id, messageType: "approval_direction", payload: { approvalId: approval.id, directionOpinion: input.directionOpinion, responseTaskId: task.id, responseArtifactRequired }, idempotencyKey: `approval-direction-${approval.id}-${task.id}`, sourceObjectType: "approval", sourceObjectId: approval.id, responseObjectType: "task", responseObjectId: task.id, traceId: `tr_direction_${approval.id}` };
      const messageResult = this.messages.create(connection, messageInput); const message = messageResult.message;
      if (messageResult.created) this.events.append(connection, "structured_message", message.messageId, 0, [this.messageEvent(message, approval.bossId)]);
      const expectedVersion = this.events.countForAggregate(connection, "approval", approval.id);
      this.events.append(connection, "approval", approval.id, expectedVersion, [{ eventType: "BossDirectionAssigned", aggregateType: "approval", aggregateId: approval.id, payload: { projectId: approval.projectId, taskId: task.id, approvalId: approval.id, responseTaskId: task.id, assignedLeadRole: lead.roleId, assignedLeadInstance: lead.instanceId, responseArtifactRequired }, inputSummary: { approvalId: approval.id }, outputSummary: { responseTaskId: task.id }, result: "success", failure: null, retryCount: 0, durationMs: 0, actor: { type: "boss", id: approval.bossId }, traceId: `tr_direction_${approval.id}`, occurredAt: now, attemptId: null, rejectionReason: null, redactionReason: "direction text is stored only as structured business input", eventCategory: "ordinary" }]);
      return { approvalId: approval.id, directionOpinion: input.directionOpinion, assignedLead: { roleId: lead.roleId, instanceId: lead.instanceId }, responseTaskId: task.id, responseArtifactRequired, messageId: message.messageId };
    });
  }
  /** 校验方向只能交给已启用的责任岗位，不能把 Boss 意见转成任意成员命令。 */
  private assertLead(actualRoleId: string, requestedRoleId: string): void { if (canonicalRoleId(actualRoleId) !== canonicalRoleId(requestedRoleId) || !DIRECTION_LEAD_ROLES.has(canonicalRoleId(actualRoleId))) throw new InvalidArgumentError("assignedLead 必须是已启用的责任组长、开发代表或责任 PM"); }
  /** 读取已经完成的交接，重复请求不重复创建任务、消息或领域事件。 */
  private readExistingConversion(connection: DatabaseConnection, approval: ReturnType<EvidenceRepository["getApproval"]>, directionOpinion: string): { approvalId: string; directionOpinion: string; assignedLead: { roleId: string; instanceId: string }; responseTaskId: string; responseArtifactRequired: boolean; messageId: string } {
    if (approval.direction && approval.direction !== directionOpinion) throw new InvalidArgumentError("该审批已经绑定不同的 Boss 方向意见");
    const task = connection.prepare("SELECT id,expected_deliverables_json FROM tasks WHERE project_id=? AND id=?").get(approval.projectId, approval.responseTaskId) as { id: string; expected_deliverables_json: string } | undefined;
    const message = connection.prepare("SELECT message_id,receiver_role,receiver_instance_id FROM structured_messages WHERE project_id=? AND source_object_type='approval' AND source_object_id=? AND response_object_id=? ORDER BY created_at DESC LIMIT 1").get(approval.projectId, approval.id, approval.responseTaskId) as { message_id: string; receiver_role: string; receiver_instance_id: string } | undefined;
    if (!task || !message) throw new InvalidArgumentError("审批已有响应任务但缺少结构化交接记录");
    return { approvalId: approval.id, directionOpinion: approval.direction ?? directionOpinion, assignedLead: { roleId: message.receiver_role, instanceId: message.receiver_instance_id }, responseTaskId: task.id, responseArtifactRequired: (JSON.parse(task.expected_deliverables_json) as string[]).includes("response-artifact"), messageId: message.message_id };
  }
  /** 生成结构化消息领域事件，事件正文只保存脱敏摘要和对象引用。 */
  private messageEvent(message: StructuredMessage, bossId: string): DomainEventDraft { return { eventType: "StructuredMessageCreated", aggregateType: "structured_message", aggregateId: message.messageId, payload: { messageId: message.messageId, projectId: message.projectId, taskId: message.taskId, messageType: message.messageType, senderRole: message.sender.roleId, receiverRole: message.receiver.roleId, responseObjectType: message.responseObjectType, responseObjectId: message.responseObjectId }, inputSummary: { messageType: message.messageType }, outputSummary: { status: message.status }, result: "success", failure: null, retryCount: 0, durationMs: 0, actor: { type: "boss", id: bossId }, traceId: message.traceId ?? `tr_message_${message.messageId}`, occurredAt: message.createdAt, attemptId: null, rejectionReason: null, redactionReason: "direction text remains in structured message payload", eventCategory: "ordinary" }; }
}
