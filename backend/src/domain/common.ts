...
export function validateSafeValue(value: string, fieldName: string): string {
  ...
  if (/^[^\x00-\x1f\x7f\s/\\]+$/.test(value) === false)
...
export function newObjectId(kind: string): string {
  if (!/^[a-z](?:[a-z0-9_-]{0,62}[a-z0-9])?$/.test(kind))
...
export function validateJsonObject(
  ...
  if (
    key &&
    /api[_ -]?key|authorization|bearer|cookie|secret|password|prompt|access[_ -]?token|refresh[_ -]?token|tokens?/i.test(
      key,
    )
  ) {
...
export function assertSafeData(value: unknown, key?: string): void {
  ...
  if (
    typeof value === "string" &&
    /(?:api[_ -]?key|authorization|bearer|cookie|secret|password|system\s+prompt|prompt|access[_ -]?token|refresh[_ -]?token|tokens?)\s*[:=]|\bbearer\s+\S+|\bsk-[A-Za-z0-9][A-Za-z0-9_-]*/i.test(
      value,
    )
  ) {
  ...
  if (Array.isArray(value)) value.forEach((item) => assertSafeData(item));
    ...
    Object.entries(value).forEach(([name, item]) => assertSafeData(item, name));
}
...
export function normalizeUtc(value: string | Date): string {
  ...
  if (!/[zZ]|[+-]\d\d:\d\d$/.test(value))
    throw new Error("datetime must include a timezone offset");
