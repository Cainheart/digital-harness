...
export type CreateMessageInput = { sender: MessageEndpoint; receiver: MessageEndpoint; projectId: string; taskId: string; messageType: MessageType; payload: Record<string, unknown>; idempotencyKey: string; sourceObjectType?: string | null; sourceObjectId?: string | null; responseObjectType?: string | null; responseObjectId?: string | null; traceId?: string | null };
...
export function parseCreateMessage(input: unknown): CreateMessageInput {
  ...
  const sourceObjectType = value.sourceObjectType == null ? null : validateSafeValue(String(value.sourceObjectType), "sourceObjectType"); const sourceObjectId = value.sourceObjectId == null ? null : validateSafeValue(String(value.sourceObjectId), "sourceObjectId"); const responseObjectType = value.responseObjectType == null ? null : validateSafeValue(String(value.responseObjectType), "responseObjectType"); const responseObjectId = value.responseObjectId == null ? null : validateSafeValue(String(value.responseObjectId), "responseObjectId"); if ((sourceObjectType === null) !== (sourceObjectId === null)) throw new InvalidMessageError("sourceObjectType 和 sourceObjectId 必须同时提供"); if ((responseObjectType === null) !== (responseObjectId === null)) throw new InvalidMessageError("responseObjectType 和 responseObjectId 必须同时提供"); const traceId = value.traceId == null ? null : validateSafeValue(String(value.traceId), "traceId");
  return { sender, receiver, projectId, taskId, messageType, payload, idempotencyKey, sourceObjectType, sourceObjectId, responseObjectType, responseObjectId, traceId };
}

/** 为合法创建请求物化新的结构化消息。 */
export function materializeMessage(input: CreateMessageInput): StructuredMessage { return { ...input, messageId: newObjectId("message"), createdAt: utcNow(), status: "pending", handledAt: null, handledBy: null, responseObjectType: input.responseObjectType ?? null, responseObjectId: input.responseObjectId ?? null, version: 1 }; }
