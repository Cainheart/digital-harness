...
export type ExecutionGrant = {
  ...
  roleVersion: number;
  taskVersion: number;
  workspaceGrant: { root: string; readOnly: boolean };
  toolPolicy: ToolName[];
  commandPolicy: { allowedCommands: string[]; forbiddenCommands: string[] };
  deadline: string;
...
export type RolePolicyInput = Pick<
  RoleDefinition,
  | "roleId"
  | "roleVersion"
  | "allowedTools"
...
export type PolicyTask = Pick<
  Task,
  "id" | "projectId" | "ownerRole" | "version"
>;
/** 策略审计写入器；拒绝结果必须进入脱敏审计，而不写模型原文。 */
export type PolicyDecisionSink = (
