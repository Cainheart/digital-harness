...
V1 的正式交付形态是面向单机、单 Boss 的可安装桌面应用，目标平台为 macOS 和 Windows。用户完成安装后应能通过桌面图标启动应用，不要求手动启动 Node.js、Vite 或浏览器。
...
AC-27. V1 必须提供可安装的 macOS 和 Windows 桌面应用；用户双击启动后能进入本地控制台，应用自动管理前端与 Node.js/TypeScript 控制面，Docker 不可用时能明确提示并阻断受影响的真实执行；应用关闭、重启或升级后不得丢失已提交数据或越过人工关卡。
...
| Node.js/TypeScript sidecar | 由桌面应用启动和管理的本地 Node.js/Fastify 控制进程 | 负责业务 API、持久化、readiness 和后续 Agent 编排 |
