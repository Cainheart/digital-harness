...
| T9-AC-COMMIT | 分支、验收与开发完成提交 | Task 开发、测试和文档完成后检查 `git branch --show-current`、`git log`、提交哈希和工作区状态 | Task 9 在从最新 `master` 创建的 `dev/task-9` 分支上完成；已创建完成提交，提交哈希已写入验收证据；验收和 Review 成功后才合并到 `master`，并记录合并提交哈希。 |

验收证据包括：浏览器录制/截图、API 请求和响应、SSE 游标、页面状态截图、理解性任务结果、错误提示和敏感信息扫描。

## 7. 完成定义与交接

- 开发结束时已在 `dev/task-9` 分支创建一次可识别的 Task 9 完成提交，提交哈希已记录在验收证据中，相关工作区无未提交变更；验收和 Review 成功后才允许合并到 `master`，并记录合并提交哈希。
